...
function inputTypeToSlotType(ty: string): string {
  // TODO also have select-one, select-multiple
  switch (ty) {
    case 'checkbox':
      return 'bool';
    case 'password':
...
function makeFormTemplate(formId: string): Promise<ItemTemplate> {
  let fieldNames: { name: string, label: string, type: string }[] = [];
  document.querySelectorAll('#' + formId + ' .meeco-slot').forEach((x: any) => {
    let res = {
      name: x.name,
      label: '',
      type: inputTypeToSlotType(x.type)
    };
  ...
  return App.templates.saveUnlessExists({
    ...
    slots_attributes: fieldNames.map(({ label, name, type }) => {
      return { label, name, slot_type_name: type }
    })
  });
}
